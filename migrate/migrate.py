...
def has(t):
    return t in {r[0] for r in cur.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
...
if has("camera"):
    c = cols("camera")
    if "recording_enabled" not in c: cur.execute("ALTER TABLE camera ADD COLUMN recording_enabled BOOLEAN DEFAULT 0")
    if "group_name" not in c: cur.execute("ALTER TABLE camera ADD COLUMN group_name VARCHAR(60)")

if has("camera_access"):
    c = cols("camera_access")
    if "enabled" not in c: cur.execute("ALTER TABLE camera_access ADD COLUMN enabled BOOLEAN DEFAULT 1")
    cur.execute("""INSERT INTO camera_access (camera_id, user_id, enabled)
        SELECT id, user_id, 1 FROM camera WHERE user_id IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM camera_access ca WHERE ca.camera_id = camera.id AND ca.user_id = camera.user_id)""")

if has("payment_request"):
    c = cols("payment_request")
    if "user_hidden" not in c: cur.execute("ALTER TABLE payment_request ADD COLUMN user_hidden BOOLEAN DEFAULT 0")
    if "admin_hidden" not in c: cur.execute("ALTER TABLE payment_request ADD COLUMN admin_hidden BOOLEAN DEFAULT 0")

n = cur.execute("SELECT COUNT(*) FROM tariff").fetchone()[0]
if n == 0:
    cur.execute("""INSERT INTO tariff (name, price, period_days, interval_seconds, max_cameras, archive_days, is_active, is_b2b, max_users) VALUES
      (?,?,?,?,?,?,1,0,1),(?,?,?,?,?,?,1,0,1),(?,?,?,?,?,?,1,0,1),(?,?,?,?,?,?,1,1,5),(?,?,?,?,?,?,1,1,3)""",
